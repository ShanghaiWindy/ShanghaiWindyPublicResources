    ...
    SubShader
    {
        Pass
        {
            ...
            v2f vert(vertexInput v)
            {
                ...
                o.stampUV -= 0.5;
                o.stampUV *= (_AlphaMapSize-1)/_AlphaMapSize;
                o.stampUV += 0.5;
                return o;
            }
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float thresh = _AlphaMapSize / (_AlphaMapSize-1) - 1;
                bool cp = (i.stampUV.x < -thresh || i.stampUV.x > thresh + 1 || i.stampUV.y < -thresh|| i.stampUV.y > thresh + 1);
                if (cp)
                ...
                float noise = 0;
                float falloff = ComputeFalloff(i.uv, stampUV, noiseUV, 0);

                #if _FALLOFFNOISE
                    noise = (Noise(falloffuv, _FalloffNoise)) / _RealSize.x;
                #elif _FALLOFFFBM
                    noise = (NoiseFBM(falloffuv, _FalloffNoise)) / _RealSize.x;
                #elif _FALLOFFWORLEY
                    noise = (NoiseWorley(falloffuv, _FalloffNoise)) / _RealSize.x;
                #elif _FALLOFFWORM
                    noise = (NoiseWorm(falloffuv, _FalloffNoise)) / _RealSize.x;
                #elif _FALLOFFWORMFBM
                    noise = (NoiseWormFBM(falloffuv, _FalloffNoise)) / _RealSize.x;
                #elif _FALLOFFNOISETEXTURE
                    noise = (tex2D(_FalloffNoiseTexture, falloffuv * _FalloffNoiseTexture_ST.xy + _FalloffNoiseTexture_ST.zw)[_FalloffNoiseChannel] * 2.0 - 1.0) / _RealSize.x * _FalloffNoise.y + _FalloffNoise.w;
                #endif
