            ...
            float CurvatureFromNormal(float2 uv, float2 scale, float lod)
            {
                ...
                float posX, negX, posY, negY;
                    ...
                    if (uvpy.y > 1)
                    {
                        uvpy.y -= 1;
                        uvpy.y += height;
                        posY = (tex2Dlod(_Normalmap_PY, uvpy) * 2.0 + 1.0).x;
                    }
                    else
                    {
                        posY = (tex2Dlod(_Normalmap, uvpy) * 2.0 + 1.0).x;
                    }
                #else
                    posY = (tex2Dlod(_Normalmap, uvpy) * 2.0 + 1.0).x;
                #endif
                    ...
                    if (uvny.y < 0)
                    {
                        uvny.y += 1;
                        uvny.y -= height;
                        negY = (tex2Dlod(_Normalmap_NY, uvny) * 2.0 + 1.0).x;
                    }
                    else
                    {
                        negY = (tex2Dlod(_Normalmap, uvny) * 2.0 + 1.0).x;
                    }
                #else
                    negY = (tex2Dlod(_Normalmap, uvny) * 2.0 + 1.0).x;
                #endif
