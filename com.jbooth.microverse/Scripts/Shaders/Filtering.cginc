...
// the tree stamp sets this to make it work. Fugly.
            ...
            float DoFilters(float2 uv, float2 stampUV, float2 noiseUV)
            {
                ...
                ApplyWeightNoise(noiseUV, stampUV, result);
                    ...
                    // TODO: For some reason normal breaks in some stamps but not others,
                    // unless we recompute it. So for now, just recompute it every time.
                    // it's fast, so if we can't fix this maybe get rid of the normal map
                    // all together to save memory?
                ...
                #if _CURVATUREFILTER
                    float curvature = SAMPLE(_Curvemap, shared_linear_clamp, uv).r;
