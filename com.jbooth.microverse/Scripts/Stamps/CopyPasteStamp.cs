...
{
    public class CopyPasteStamp : Stamp, IHeightModifier, ITextureModifier
...
#endif
    {
        ...
        public void ApplyTreeStamp(TreeData vd, Dictionary<Terrain, List<TreeJobHolder>> jobs, OcclusionData od)
        {
                ...
                stamp.treeData.randomsTex != null && stamp.treeData.positonsTex != null)
            {
                ...
                NativeArray<uint> indexes = new NativeArray<uint>(indexMap.Length, Allocator.Persistent);
                for (int i = 0; i < indexMap.Length; ++i)
                {
                    indexes[i] = (uint)indexMap[i];
                }
