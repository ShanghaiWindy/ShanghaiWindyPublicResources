...
namespace JBooth.MicroVerseCore
{
    public class JumpFloodSDF
    {
        public static RenderTexture CreateTemporaryRT(Texture source, int channel = 0, float zoom = 1, int downscale = 1)
        {
            ...
            w /= downscale;
            h /= downscale;
            var output = RenderTexture.GetTemporary(w, h, 0, RenderTextureFormat.R16);
        ...
        static RenderTexture Generate(Texture source, RenderTexture output, int channel, float zoom, int downscale)
        {
            ...
            RenderTexture rtA = RenderTexture.GetTemporary(source.height, source.width, 0, RenderTextureFormat.RG16);
            RenderTexture rtB = RenderTexture.GetTemporary(source.height, source.width, 0, RenderTextureFormat.RG16);
