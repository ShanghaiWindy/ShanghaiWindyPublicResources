    ...
    class CopyPasteStampEditor : Editor
    {
        ...
        public static void Capture(CopyPasteStamp cpStamp, string path)
        {
            ...
            foreach (var terrain in terrains)
            {
                ...
                if (!cpStamp.GetBounds().Intersects(TerrainUtil.ComputeTerrainBounds(terrain)))
                    continue;
                Vector2 uv = new Vector2(ourPos.x, ourPos.z);
                copyMat.SetVector("_UVCenter", uv);
            ...
            var cp = CopyStamp.Create(heightTex, indexTex, weightTex, terrainLayers, renorm, tcd, dcd);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Create Copy Object"))
            {
