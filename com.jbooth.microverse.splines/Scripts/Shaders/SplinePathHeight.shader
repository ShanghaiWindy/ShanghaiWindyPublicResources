    ...
    SubShader
    {
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float3 data = tex2D(_SplineSDF, i.uv).xyz;
                float4 os = _SplineSDF_TexelSize;
                float falloff = Blend(_Width, _Smoothness, data.g);

                #if _HEIGHTNOISE
                    data.b += ((Noise(noiseUV, _HeightNoise) * falloff)) / _RealHeight;
                #elif _HEIGHTFBM
                    data.b += ((NoiseFBM(noiseUV, _HeightNoise) * falloff)) / _RealHeight;
                #elif _HEIGHTWORLEY
                    data.b += ((NoiseWorley(noiseUV, _HeightNoise) * falloff)) / _RealHeight;
                #elif _HEIGHTWORM
                    data.b += ((NoiseWorm(noiseUV, _HeightNoise) * falloff)) / _RealHeight;
                #elif _HEIGHTWORMFBM
                    data.b += ((NoiseWormFBM(noiseUV, _HeightNoise) * falloff)) / _RealHeight;
                #elif _HEIGHTNOISETEXTURE
                    float hnoise = abs(tex2D(_HeightNoiseTexture, noiseUV * _HeightNoiseTexture_ST.xy + _HeightNoiseTexture_ST.zw)[_HeightNoiseChannel] * 2.0 - 1.0) / _RealHeight * _HeightNoise.y + _HeightNoise.w;
                   ...
                   nstr = 1 - (1-nstr) * (1-nstr);
                ...
                #if _FALLOFFNOISE
                    data.b -= abs((Noise(noiseUV, _FalloffNoise) * nstr)) / _RealHeight;
                #elif _FALLOFFFBM
                    data.b -= abs((NoiseFBM(noiseUV, _FalloffNoise) * nstr)) / _RealHeight;
                #elif _FALLOFFWORLEY
                    data.b -= abs((NoiseWorley(noiseUV, _FalloffNoise) * nstr)) / _RealHeight;
                #elif _FALLOFFWORM
                    data.b -= abs((NoiseWorm(noiseUV, _FalloffNoise) * nstr)) / _RealHeight;
                #elif _FALLOFFWORMFBM
                    data.b -= ((NoiseWormFBM(noiseUV, _FalloffNoise) * nstr)) / _RealHeight;
                    ...
                    data.b -= (fnoise * nstr);
                #endif
                data.b -= _TerrainHeight/_RealHeight;
                data.b = lerp(data.b, data.b-(_Trench/_RealHeight), 1.0 - saturate(smoothstep(_Width, _Width+_Smoothness, data.g)));
                float newHeight = saturate(lerp(height, data.b, falloff));
