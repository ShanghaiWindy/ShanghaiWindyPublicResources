...
namespace JBooth.MicroVerseCore
{
    ...
    public struct UnpackTreeInstanceJob : IJob
    {
        ...
        [ReadOnly] public NativeArray<uint> treeIndexes;
        ...
        public void Execute()
        {
            for (int i = 0; i < placementData.Length; ++i)
            {
                ...
                if (pd.w > 0)
                {
                    ...
                    tree.prototypeIndex = (int)treeIndexes[(int)rd.x % treeIndexes.Length];
    ...
    public class TreeJobHolder
    {
        ...
        private NativeArray<uint> treeIndexes;
        ...
        public void AddJob(RenderTexture filteredInstances, RenderTexture randomResults, NativeArray<uint> treeIndexes)
        {
    ...
    public class TreeStamp : Stamp, ITreeModifier, ITextureModifier
    {
        ...
        public void ApplyTreeStamp(TreeData td, Dictionary<Terrain, List<TreeJobHolder>> jobs, OcclusionData od)
        {
            ...
            material.SetFloatArray(_TextureLayerWeights, textureLayerWeights);
        ...
        public void ProcessTreeStamp(TreeData vd, Dictionary<Terrain, List<TreeJobHolder>> jobs, OcclusionData od)
        {
            ...
            if (prototypes.Count == 0)
                return;
            UnityEngine.Profiling.Profiler.BeginSample("Process Tree Stamp");
            UnityEngine.Profiling.Profiler.BeginSample("Height/Texture Mods");
            ...
            if (heightModAmount != 0)
            {
                ...
                RenderTexture.ReleaseTemporary(vd.heightMap);
                vd.heightMap = nhm;
            }
            if ((layer != null && layerWeight > 0))
            {
                ...
                RenderTexture.ReleaseTemporary(vd.indexMap);
                RenderTexture.ReleaseTemporary(vd.weightMap);
                vd.indexMap = nim;
                vd.weightMap = nwm;
            ...
            for (int i = 0; i < prototypes.Count; ++i)
            {
                indexes[i] = (uint)prototypeIndexes[i];
            }
