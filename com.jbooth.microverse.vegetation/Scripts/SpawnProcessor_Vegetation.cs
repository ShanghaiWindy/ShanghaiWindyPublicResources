    ...
    public partial class SpawnProcessor
    {
        ...
        void RenderVegetationClearLayers(Terrain[] terrains, MicroVerse.DataCache dataCache)
        {
            ...
            foreach (var terrain in terrains)
            {
                ...
                TreeData td = new TreeData(terrain, heightmapGen, normalGen, curvatureGen, indexMap, weightMap, clearMap);
                dataCache.treeDatas[terrain] = td;

                DetailData dd = new DetailData(terrain, heightmapGen, normalGen, curvatureGen, indexMap, weightMap, clearMap);
            ...
            foreach (var terrain in terrains)
            {
                ...
                if (dataCache.treeDatas.ContainsKey(terrain))
                {
                    if (terrainBounds.Intersects(treeModifier.GetBounds()))
                    {
                        ...
                        // because tree's can modify height maps and splat maps

                        dataCache.heightMaps[terrain] = td.heightMap;

                        dataCache.indexMaps[terrain] = td.indexMap;
                        dataCache.weightMaps[terrain] = td.weightMap;
